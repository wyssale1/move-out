...
const backendModel = {
    ...
    generateCategory(cat, data) {
        if (data) {
            ...
            let categorieName = backendModel.replaceUmlaute(categories[cat].toLowerCase())
            container.querySelector(".category").dataset.id = cat
            container.querySelector("img").src = "img/" + categorieName + ".svg"
            container.querySelector("img").alt = categorieName
...
elements.logoutBtn.addEventListener("click", databaseModel.logoutFunc)
